...
_unit spawn {
	...
	if (Life_request_timer) then {_maxTime = time + (life_respawn_timer * 150);waitUntil {_Timer ctrlSetText format["STR_Medic_Respawn",[(_maxTime - time),"MM:SS.MS"] call BIS_fnc_secondsToString]; round(_maxTime - time) <= 0 || isNull _this};};Life_request_timer = false; //resets increased respawn timer
